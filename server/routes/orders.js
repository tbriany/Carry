...
router.post("/add", async (req, res, next) => {
    try {
        const { order_status, required_date,customer_id, store_id, courier_id, delivery_fee, total } = req.body;
        const response = await ordersQueries.addOrder({ order_status, 
            required_date, 
            customer_id, 
            store_id, 
            courier_id, 
...
router.get('/courier_orders/:courier_id', async (req, res, next)=>{
    try{
        ...
        const orderByCourierId = await ordersQueries.getOrderByCourierId(courier_id)
        res.status(200).json({
